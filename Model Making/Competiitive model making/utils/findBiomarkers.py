...
def findBiomarkers(model, fvaRxns=[], mods=[], mode='', always_unite=False, synchronous=False,
                   eps=1.0, threshold=0.1, fracOpt=0, forceFlux=True, geneAssociationByKO=True):
    ...
    if mode == 'IEMgene':
        ...
        if geneAssociationByKO:
            ...
            affectedRxns = cobra.manipulation.knock_out_model_genes(model1, mods)
    ...
    # sort and remove biomarkers based on the threshold and the biomarker 'score'
    if threshold != 0:
